...
import { DirectMessage, Message } from "@/models/BashModels";
...
export async function GET(req: Request) {
  try {
    ...
    if (!conversationId) {
      return new NextResponse("Convo ID missing", { status: 400 });
    }

    let messages = [];

    const findQuery: any = { conversationId };
    ...
    messages = await DirectMessage.find(findQuery)
      .populate({
        path: "memberId",
