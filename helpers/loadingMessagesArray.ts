export const loadingMessagesArray = [
 ...
 export const pickRandomLoadingString = (loadingMessagesArray: any) => {
    // Check if the array is empty
    if (!loadingMessagesArray || loadingMessagesArray.length === 0) {
      throw new Error("Array cannot be empty");
    }
  
    // Get a random index between 0 and the array length minus 1
    const randomIndex = Math.floor(Math.random() * loadingMessagesArray.length);
  
    // Return the string at the random index
    return loadingMessagesArray[randomIndex];
  }
